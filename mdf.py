...
def desa_img(artname, url_file):
    """
    Stores jpg images in a folder
    :param artname: name of the image file
    :param url_file: url that contains the image
    """
...
def extreu_text(url_article):
    """
    Tidies and extracts text field from URL
    :param url_article: URL that contains the text
...
def nuvol_tags(text_conv):
    """
    Generates tag cloud image
    :param text_conv: source text in string format
    :return: image as .png
    """
...
for i, article in enumerate(soup_body.find_all('article')):
    ...
    # Mostrar per pantalla el procés d'obtenció de cada article
    print('text of {} processed'.format(tag.string))
